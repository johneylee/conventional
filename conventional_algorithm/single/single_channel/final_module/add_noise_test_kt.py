...
snr = [20, 15, 10, 5, 0, -5]
ntype = ["aurora4_airport_1", "aurora4_lobby_1", "aurora4_benz_1", "aurora4_exhibition_1", "aurora4_station_1", "babble", "destroyerengine", \
         "destroyerops", "DLIVING", "factory1", "leopard", "pink", "SPSQUARE", "STRAFFIC", "volvo"]

for k in snr:
    for j in ntype:
        # txt 리스트 불러와서 한번에 처리하기
        # file = open("/home/leesunghyun/Downloads/TIMIT//TEST/all_wav.txt", "r")
        file = open("/home/sunghyunlee/workspace/data/ksponspeech/eval_wav/kor_ai_speech_full", "r")
        ...
        for i in range(len(data)):
            ...
            sampling_rate, noise = wav.read('{}.wav'.format(j))
            speech = speech / 32768.0
            noise = noise / 32768.0
            noisy = add_noise(speech, noise, k)

            name = data[i].split('.')
            # filepath = '/home/sunghyunlee/workspace/data/ksponspeech/eval_wav/noisy/' + '{}'.format(ntype[1]) + '/{}dB'.format(snr[4])     # change file path
            filepath = '/home/sunghyunlee/workspace/data/ksponspeech/eval_wav/noisy/' + '{}'.format(j) + '/{}dB'.format(k)  # change file path
            ...
            # print(filepath + os.sep + file_name + '_{}_{}dB.wav'.format(j, k), sr, np.asarray(noisy * 32768, dtype=np.int16))
            scipy.io.wavfile.write(filepath + os.sep + file_name + '_{}_{}dB.wav'.format(j, k), sr, np.asarray(noisy * 32768, dtype=np.int16))
