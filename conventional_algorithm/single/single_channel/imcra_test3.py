...
class ImcraTest:
    ...
    def __init__(self, y_PSD):
        ...
        self.alpha = {'0':0.92, 's':0.5, 'd':0.85}
        ...
        self.n_PSD[0, :] = self.y_PSD[0, :]                         # y_psd 0번째를 noise psd 0번째와 1번째로 초기화
        self.n_PSD[1, :] = self.y_PSD[0, :]
        self.s_f = np.convolve(y_PSD[0, :], self.parameter['wsub'], 'same')      # 0번째 프레임에 대한 Sf - initialize
        self.s_hat = self.s_f
        self.s_bar = self.s_f
        self.s_hat_sub = self.s_f
        self.s_bar_sub = self.s_f

        for j_frm in range(self.parameter['u']):                  # S_hat_min 값을 u 개의 sub window 내에서 구하겠다??
    ...
    def SNR_parameter(self, i_frm):
        ...
        gamma = self.y_PSD[i_frm, :] / self.n_PSD[i_frm, :]           # a posteriori SNR = noisy_P / noise_P
        self.xi = self.alpha['0'] * (self.Gi ** 2) * self.gamma_prev + (1 - self.alpha['0']) * np.maximum(gamma - 1, 0)    # Decision-directed, modified version - om lsa
        self.nu = gamma * self.xi / (1 + self.xi)                          # nu 정의
        self.Gi = self.xi / (1 + self.xi) * np.exp(exp1(self.nu) / 2)           # Gi: spectral gain function - om lsa

        return gamma, self.xi, self.nu

    """eq15"""
    def first_smoothing(self, s_f):
        self.s_hat = self.alpha['s'] * self.s_hat + (1 - self.alpha['s']) * s_f       # recursive averaging in time domain smoothing
    ...
    def finalization(self, ind, gamma):
        if ind == self.parameter['v']:
            self.s_hat_sub_min[1:self.parameter['u'], :] = self.s_hat_sub_min[:self.parameter['u'] - 1, :]  # v sample 이 될때마다 u sub window 내에서 minimum 값이 결정되고 저장된다.
    ...
    def noise_estimation_process(self):
        ...
        for i_frm in range(1, self.n_frm - 1):  # 첫번째와 마지막 프레임은 제외하고 for 문 돌리겠다.
            ind = i_frm % self.parameter['v'] + 1  # v의 크기만큼이 되면 갱신시켜주기 위한 부분
            s_f = self.initialize_smoothing(i_frm)
            gamma, xi, nu = self.SNR_parameter(i_frm)
            s_hat = self.first_smoothing(s_f)
